...
**Orion is a control plane for agents, not a generation engine.** It makes **no generation or proof inference calls** — producing artifacts (code/tests/instrumentation) and computing proof verdicts are delegated to the sandboxed fleet under the developer's own authentication — but it **may make bounded coordinator-role calls** (embeddings, recall/relevance ranking, memory summarization, drift/anchor scoring, lightweight intent/plan reasoning) via a *configured, swappable provider* (local default, cloud opt-in). See the **Bounded Coordinator Inference** section below and `docs/adr/ADR-0001-bounded-coordinator-inference.md`. It **spawns and drives the developer's own coding agent** — Claude Code by default, or any ACP-compliant agent — over the **Agent Client Protocol (ACP)**, using *that agent's* existing authentication (e.g. a Claude Max/Pro login; optional per-preset `ANTHROPIC_BASE_URL`/key override). The chat "brain" is therefore pluggable; Orion's contribution is the layer around it. Behind the chat, Orion solves the hard problems the manifesto names — efficient agent coordination, context/memory management, durable task tracking, and above all the deterministic **multi-modal proof harness** that gates completion.
...
## PRINCE-Derived Refinements (Bayer corroboration)

> Added 2026-06-22 from the Bayer PRINCE case study (*Building Reliable Agentic AI Systems*, martinfowler.com; full synthesis: [docs/research/Bayer-PRINCE-reliable-llm-2026-06-22.md](../research/Bayer-PRINCE-reliable-llm-2026-06-22.md)). PRINCE is an in-production, **regulated-industry** multi-agent RAG system; its reliability bar is *faithful, cited answers*, Orion's is *independent multi-modal proof*. We adopt PRINCE's **engineering mechanisms**, never its **metrics** as a ship gate — RAGAS faithfulness/relevancy is right for QA, far too weak for code. PRINCE independently validates Orion's load-bearing bets (reliability from the harness+context not the model; LLM-as-judge never sets the verdict; resumable checkpointed state; per-stage testing-pyramid eval with tiered golden data; provider fallback + circuit breakers; self-instrumented traffic). These two refinements adopt the practices that go *beyond* what the PRD already specified; each amends the named section.

- **Longitudinal harness eval — continuous live-run evaluation of Orion's own judgment** *(amends Harness Reliability Requirements + Testing Decisions / SkillEval; V2.0 foundation → V2.3 gate).* PRINCE runs **two** eval tiers: change-triggered dataset evals **and** a daily batch eval over real production traffic. Orion has tier 1 (change-triggered cassette/golden **SkillEval**) and a promotion-scoped **canary** (N runs around one self-evolution promotion) — but **no tier 2**: nothing watches the *longitudinal trend* of the deployed loop's own LLM-step quality on real work, across model swaps and skill versions. Because Orion is **model-agnostic by construction** and its skills **self-evolve**, this is a silent-regression surface the **per-run proof gate structurally cannot see** (the proof gate protects each *artifact*; it never reports "the completeness gate degraded 15% since the model swap"). Add a continuous/periodic eval pass that consumes the existing **self-instrumentation audit trail** (the data already exists) and reports per-skill quality trend — stratified by model + skill version — from **harness-derived signals only** (mutation-score delta, empirical pass-rate, coverage-gate pass-rate, attempt-to-verdict ratio, drift incidence; never agent self-report). Verdict stays deterministic (per the Hybrid-eval refinement); a statistically-significant downward trend is an escalation. This is the missing complement to SkillEval (pre-deploy) and the canary (per-promotion): run-stream-wide, continuous, model/skill-version-stratified.

- **Evidence-sufficiency reflection gate before generation** *(amends Phase E + `context-engine`; V2.0–V2.1).* PRINCE runs **three** complementary reflection loops — **process** (Think&Plan: "is this trajectory right?"; credited with a *dramatic* tool-selection-accuracy gain), **data/evidence-sufficiency** ("is what I gathered enough? if not, retrieve more, *then* proceed"), and **draft** (output review). Orion already has the process analog (drift/re-anchor E14, confidence E15) and a far stronger output check than draft-review (the **proof harness**) — but it is **missing the evidence-sufficiency loop**. Insert a cheap pre-dispatch gate between **E2** (assemble `ContextBundle`) and **E3** (dispatch generator): given the bundle + the Task's `ProofObligation`, decide whether evidence is **sufficient** to attempt generation, or whether identified gaps require another recall cycle (Context-Store/memory retrieval, a Polaris pull, or a human clarification) **before** spending a generation+proof attempt. Today insufficiency is found only **reactively** via proof-reject-reloop (E13) — the most expensive possible point. The gate is **HYB** (LLM proposes gaps; deterministic check that required obligation inputs / spec dimensions are present in the bundle), must stay **generation-domain-clean** (reads only bundle + obligation; never the held-out corpus; never sets a verdict — Trust invariants 1–3, 7), and is **bounded** (`context.max_sufficiency_cycles`, budget-charged) so it cannot loop forever. Net effect: fewer wasted generation/proof attempts and earlier human-escalation on genuinely under-specified tasks.

> **Considered, deferred (see the synthesis doc):** the PRINCE **hybrid-retrieval recipe** (metadata pre-filter → query expansion → weighted fusion → cross-encoder rerank) folds into the `context-engine` C4/C6 retrieval implementation when built; **NER + confidence-gated metadata enrichment** is a template for V2.1 brownfield code-map intake; **draft-reflection** (cheap pre-proof generation-side self-review) is a lower-priority token-saver that must never become a shadow verdict. **Not adopted:** RAGAS metrics as any completion criterion (the proof harness is the gate).

## Bounded Coordinator Inference

> Added 2026-06-23 (epic or-hd3, slice 0; full rationale: [docs/SPEC/Orion-Memory-Recall-Design.md §2](../SPEC/Orion-Memory-Recall-Design.md); decision recorded in [docs/adr/ADR-0001](../adr/ADR-0001-bounded-coordinator-inference.md)). **This section amends the core thesis** (Solution; module `agent-runtime`): the original absolute — *"holds no API key and makes no inference calls"* — is reformulated to the bounded invariant below. The memory subsystem needs embeddings, and Orion-as-coordinator benefits from in-process reasoning; this is already partly reality (a native LLM Conductor is selected when a key is present, and `safeenv` already scrubs that key from proof execs). The structural proof walls are unaffected — coordinator reasoning was never a proof input.

> **Reformulated invariant.** Orion is a control plane, not a generation engine. It makes **no generation or proof inference calls** — producing artifacts (code/tests/instrumentation) and computing proof verdicts are delegated to the sandboxed fleet under the developer's auth and gated by independent proof. Orion **may make bounded coordinator-role calls** directly — embeddings, recall/relevance ranking, memory summarization, drift/anchor scoring, and lightweight intent/plan reasoning — via a **configured, swappable provider** (local default, cloud opt-in). Coordinator output is **control-domain context only**: never a shippable artifact, never a proof input. The commodity-model principle applies to coordinator calls too — no requirement may depend on a specific model.
...
| Drift / anchor scoring | **Orion direct** | cheap-first |
...
**Trust-domain treatment.** A new provenance, `coordinator`, joins `human | proof | generation`. Coordinator output is control-domain context only — never a proof input (proof reads the spec directly, Trust invariant 7), never a shippable artifact. A coordinator *summary of a proof fact* is control-tier context; a coordinator *summary of a generation narrative* stays `generation`-tier (quarantined).

**Security.** The coordinator provider key lives in the OS keychain / encrypted-`0600`, is loaded via `safeenv`, and is **never reachable from inside the sandbox** — `safeenv` is deny-by-default (toolchain allowlist only), so the key is dropped from every proof exec by construction (reuses the Polaris-credential pattern in §Security Requirements; see or-5ym). Generated/proof execs keep running under the scrubbed env.

**Cost governance.** Coordinator-call spend (tokens + dollars + wall-clock) is tracked by the budget accountant exactly like fleet spend (amends §Resource & Cost Governance) — coordinator inference is not free and is surfaced live in the TUI.

...
2. **`orchestrator` (the Conductor)** — a **spawned, primed ACP agent** (the first agent, à la Gastown's Mayor) + a thin lifecycle manager — *not* special-cased code. Its orchestration personality lives in a **role template** (intent intake, completeness gate, decomposition, dispatch, drift/re-anchor, escalation); it exposes the ACP agent interface to the TUI and manages the specialist fleet. **It reasons and delegates; it does not compute proof verdicts or override deterministic gates** — `truth-align`/Converge, the deployment bar (`delivery`), leases/merge (`integration`/`worktree`), and dry-run/reversibility (`sandbox`) are separate deterministic tools it invokes but cannot influence (the reasoning⊥execution split; resolves the round-2 god-object finding). The lifecycle manager keeps a pid/agent file + cleanup veto while alive. See [Orion-TUI-ACP-Conductor](../SPEC/Orion-TUI-ACP-Conductor.md).
...
18. **`cmd/orion`** — the single binary (**cobra** CLI + **bubbletea** TUI): interactive TUI **and** non-interactive CLI subcommands (the rvl-cli-parity surface + the loop-control surface used by the acceptance criteria), plus `orion conductor {start|stop|attach|status|restart|acp}` lifecycle subcommands (maps `gt mayor`).

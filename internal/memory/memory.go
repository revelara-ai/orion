...
func (s *Store) Retrieve(ctx context.Context, query string, tiers ...Tier) ([]Item, error) {
	...
	rows, err := s.db.QueryContext(ctx,
		`SELECT id, tier, kind, content, content_hash, pinned, trust_tier, heat
	...
	for rows.Next() {
		var it Item
		var pinned int
		if err := rows.Scan(&it.ID, &it.Tier, &it.Kind, &it.Content, &it.Hash, &pinned, &it.TrustTier, &it.Heat); err != nil {
			return nil, err
		}
		it.Pinned = pinned == 1
...
func (s *Store) EvictToCapacity(ctx context.Context, tier Tier, keep int) error {
	...
	// Single statement: delete cold, non-pinned items beyond the hottest `keep`.
	// Pinned items are never in the candidate set (anti-erosion).
	_, err := s.db.ExecContext(ctx,
		`DELETE FROM memory_items
		 WHERE tier=? AND pinned=0 AND id NOT IN (
		   SELECT id FROM memory_items WHERE tier=? AND pinned=0 ORDER BY heat DESC LIMIT ?
		 )`, string(tier), string(tier), keep)
	if err != nil {

...
- Record each answer with record_answer — but ONLY for a single scalar value (one concrete value: a port, a format, a language, an engine, a route — whatever the decision asks, never conditional prose).
...
- Immediately after ratify_spec succeeds, call build_service to build the project to the spec and prove it in one shot. The build's phase report is shown to the developer as a card — do NOT repeat it; just briefly confirm the outcome in one line (and never claim success unless the verdict says Accept).
...
A proven change is committed on a REVIEW branch, not on the base branch (main) — the developer reviews, then decides. Ask ONCE, consolidated: "land it, close the issue, and clean up the branch?" — then call finish_change (branch, optional issue_id): it fast-forwards, closes the cited issue, and reclaims the worktree+branch in one step. NEVER split this into separate land / close / cleanup questions. (Under ORION_POST_PROOF=auto the pipeline already landed it — the report says so; don't re-ask.) For other git work the developer asks for, use the git tool directly:
...
## Working the backlog (or-cnl2)
- Picking an issue is ONE cheap step: when the developer asks what to work on, run bd ready ONCE, present the top 3 unblocked issues with a one-line why-this each, and END YOUR TURN. Do not read code, run git, or run tests before the developer picks — orientation happens AFTER they choose.
- NEVER run the full test suite in-session (go test ./... or any multi-package run). The regression gate owns baselines: it runs the suite authoritatively, memoizes green baselines, and re-proves your change independently — an in-session run is a 25-minute duplicate that cannot substitute for the gate, times out under the tool budget, and re-prompts the developer for nothing. Run at most ONE package's tests when you need a fast signal about code you just touched.

...
func NativeGrillAgent(provider llm.Provider) orchestrator.GrillAgent {
	return func(ctx context.Context, intent string, resolved map[string]string, floor []completeness.OpenDecision) ([]completeness.OpenDecision, error) {
		...
		resp, err := provider.Chat(ctx, llm.ChatRequest{
			...
			Tools:     []llm.Tool{tool},
			Messages:  []llm.Message{llm.TextMessage(llm.RoleUser, renderGrillTask(intent, resolved, floor))},
			MaxTokens: 1500,
		})
		if err != nil {
			return nil, err
...
func renderGrillTask(intent string, resolved map[string]string, floor []completeness.OpenDecision) string {

...
func TestNativeGeneratorBuildsArbitraryService(t *testing.T) {
	...
	gs := sandbox.GenSpec{Route: "/greet", Format: "json", Cases: cases}
	...
	dir := t.TempDir()
	art, err := NativeGenerator(gen)(context.Background(), gs, dir)
	if err != nil {
...
func TestNativeGeneratorRejectsPathEscape(t *testing.T) {
	...
	_, _ = NativeGenerator(gen)(context.Background(), sandbox.GenSpec{Route: "/x", Format: "json"}, dir)

...
func TestGenerateArtifactInWorktree(t *testing.T) {
	...
	art, err := GenerateGoTimeService(worktree, GenSpec{Route: "/time", Port: 8080, Format: "json", TimeZone: "UTC"})
	if err != nil {
...
func TestGeneratedServiceConformsToContract(t *testing.T) {
	worktree := t.TempDir()
	if _, err := GenerateGoTimeService(worktree, GenSpec{Route: "/now", Port: 9090, Format: "json", TimeZone: "UTC"}); err != nil {
		t.Fatalf("generate: %v", err)
	}
